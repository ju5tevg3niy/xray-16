...
{
    int res = gcd_init_qr2(nullptr, GAMESPY_GAMEID);
    if (res == -1)
    {
...
public:
    ...
    GSAuthContext(ClientAuthCallback& auth, ClientReauthCallback& reauth) : Auth(auth), Reauth(reauth) {}
};
                                   ...
                                   ClientAuthCallback& authCallback, ClientReauthCallback& reauthCallback)
{
    ...
    gcd_authenticate_user(
        GAMESPY_GAMEID, localid, userip, challenge, response, GSClientAuthCallback, GSClientReauthCallback, &ctx);
}
