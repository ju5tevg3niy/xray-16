...
private:
    ...
    bool build_line_trajectory(const STrajectoryPoint& start, const STrajectoryPoint& dest, u32 vertex_id,
        std::vector<STravelPathPoint>* path, const u32 velocity);
    ...
    bool build_trajectory(STrajectoryPoint& start, STrajectoryPoint& dest, const SCirclePoint tangents[4][2],
        const u32 tangent_count, std::vector<STravelPathPoint>* path, float& time, const u32 velocity1,
        const u32 velocity2, const u32 velocity3);
    bool compute_trajectory(STrajectoryPoint& start, STrajectoryPoint& dest, std::vector<STravelPathPoint>* path,
        float& time, const u32 velocity1, const u32 velocity2, const u32 velocity3,
        const EDirectionType direction_type);
    bool compute_path(STrajectoryPoint& start, STrajectoryPoint& dest, std::vector<STravelPathPoint>* m_tpTravelLine,
        const std::vector<STravelParamsIndex>& m_start_params, const std::vector<STravelParamsIndex>& m_dest_params,
    ...
    bool init_build(const std::vector<u32>& level_path, u32 intermediate_index, STrajectoryPoint& start,
    ...
    void postprocess_key_points(const std::vector<u32>& level_path, u32 intermediate_index, STrajectoryPoint& start,
        STrajectoryPoint& dest, std::vector<STravelParamsIndex>& finish_params, const u32 straight_line_index,
        const u32 straight_line_index_negative);
    void build_path_via_key_points(STrajectoryPoint& start, STrajectoryPoint& dest,
        std::vector<STravelParamsIndex>& finish_params, const u32 straight_line_index,
        const u32 straight_line_index_negative);
