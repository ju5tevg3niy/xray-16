...
void CStalkerActionGetOutOfAnomaly::execute()
{
    ...
    typedef std::vector<ALife::_OBJECT_ID> ids_type;
    ids_type const& restrictions = alife_object->m_dynamic_in_restrictions;

    std::vector<IGameObject*>::const_iterator I = object().feel_touch.begin();
    std::vector<IGameObject*>::const_iterator E = object().feel_touch.end();
    for (; I != E; ++I)
    {
