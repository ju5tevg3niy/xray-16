...

{
 public:
  ...
  IC bool r_chunk_safe(
      u32 ID,
      void* dest,
      size_t dest_size)  // чтение XR Chunk'ов (4b-ID,4b-size,??b-data)
  {
    const size_t dwSize = ((implementation_type*)this)->find_chunk(ID);
    if (dwSize != 0) {
...
class IReader : public IReaderBase<IReader> {
 ...
 public:
  IC IReader() : data(nullptr), Pos(0), Size(0), iterpos(0) {}
 ...
 public:
  ...
  // Same as r_string but with the difference that it returns 'false' if the
  // read string is longer than 'tgt_sz' and 'true' if it is shorter
  [[nodiscard]]
 ...
 public:
  ...
  IReader* open_chunk_iterator(u32& ID,
                               IReader* previous = nullptr);  // NULL=first
