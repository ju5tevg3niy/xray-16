...
struct Fcolor {
  ...
  constexpr u32 get_windows()
      const noexcept  // Get color as a Windows DWORD value.
  {
  ...
  constexpr Fcolor& set_windows(
      u32 dw) noexcept  // Set color from a Windows DWORD color value.
  {
  ...
  constexpr Fcolor& adjust_contrast(
      float f) noexcept  // >1 - contrast will be increased
  {
    r = 0.5f + f * (r - 0.5f);
    g = 0.5f + f * (g - 0.5f);
    b = 0.5f + f * (b - 0.5f);
  ...
  constexpr Fcolor& sub_rgb(float s) noexcept {
    r -= s;
    g -= s;
    b -= s;
    return *this;
  }
  constexpr Fcolor& add_rgb(float s) noexcept {
    r += s;
    g += s;
    b += s;
    return *this;
  }
  constexpr Fcolor& add_rgba(float s) noexcept {
    r += s;
    g += s;
    b += s;
    a += s;
  ...
  constexpr Fcolor& mul_rgba(float s) noexcept {
    r *= s;
    g *= s;
    b *= s;
    a *= s;
  ...
  constexpr Fcolor& mul_rgb(float s) noexcept {
    r *= s;
    g *= s;
    b *= s;
    return *this;
  }
  constexpr Fcolor& mul_rgba(const Fcolor& c, float s) noexcept {
    r = c.r * s;
    g = c.g * s;
    b = c.b * s;
    a = c.a * s;
  ...
  constexpr Fcolor& mul_rgb(const Fcolor& c, float s) noexcept {
    r = c.r * s;
    g = c.g * s;
    b = c.b * s;
    return *this;
  ...
  // SQ magnitude
  constexpr float magnitude_sqr_rgb() const noexcept {
    return r * r + g * g + b * b;
  }
  ...
  constexpr Fcolor& lerp(const Fcolor& c1,
                         const Fcolor& c2,
                         const Fcolor& c3,
                         float t) noexcept {
    if (t > .5f)
...
IC bool _valid(const Fcolor& c) noexcept {
