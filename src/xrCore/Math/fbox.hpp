...
struct Fbox3 {
 public:
  ...
  ICF bool contains(float x, float y, float z) const {
    return (x >= x1) && (x <= x2) && (y >= y1) && (y <= y2) && (z >= z1) &&
           (z <= z2);
  }

  ICF bool contains(const Fvector3& p) const { return contains(p.x, p.y, p.z); }

  ICF bool contains(const Fbox3& b) const {
    return contains(b.vMin) && contains(b.vMax);
  }

  [[nodiscard]] bool similar(const Fbox3& b) const {
    return vMin.similar(b.vMin) && vMax.similar(b.vMax);
  }
  ...
  ERP_Result Pick2(const Fvector3& origin,
                   const Fvector3& dir,
                   Fvector3& coord) {
    bool Inside = true;
    Fvector3 MaxT;
    ...
    if (IntRref(MaxT[WhichPlane]) & 0x80000000)
      return rpNone;
    ...
    if (0 == WhichPlane) {
      ...
      if ((coord[1] < vMin[1]) || (coord[1] > vMax[1]))
        return rpNone;

      coord[2] = origin[2] + MaxT[0] * dir[2];

      if ((coord[2] < vMin[2]) || (coord[2] > vMax[2]))
        return rpNone;
    ...
    if (1 == WhichPlane) {
      ...
      if ((coord[0] < vMin[0]) || (coord[0] > vMax[0]))
        return rpNone;
  ...
  void getpoint(int index, Fvector3& result) const {
    switch (index) {
      case 0:
        result.set(vMin.x, vMin.y, vMin.z);
        break;
      case 1:
        result.set(vMin.x, vMin.y, vMax.z);
        break;
      case 2:
        result.set(vMax.x, vMin.y, vMax.z);
        break;
      case 3:
        result.set(vMax.x, vMin.y, vMin.z);
        break;
      case 4:
        result.set(vMin.x, vMax.y, vMin.z);
        break;
      case 5:
        result.set(vMin.x, vMax.y, vMax.z);
        break;
      case 6:
        result.set(vMax.x, vMax.y, vMax.z);
        break;
      case 7:
        result.set(vMax.x, vMax.y, vMin.z);
        break;
      default:
        result.set(0, 0, 0);
        break;
