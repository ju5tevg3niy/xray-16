...
Fquaternion& Fquaternion::set(const Fmatrix& M) {
  ...
  if (trace > 0.0f) {
    ...
    // in the unusual case the original trace fails to produce a good sqrt, try
    // others...
    switch (biggest) {
      case A:
...
/* Commented out, since it's currently unused (only use is commented out in
xrPhysics) void twoq_2w(const Fquaternion& q1, const Fquaternion& q2, float dt,
Fvector& w) noexcept
{
        //
        ...
        float cosinus = q1.w*q2.w + v1.dotproduct(v2);//q1.w*q2.w+
q1.v.dotproduct(q2.v) w.crossproduct(v1, v2);
        //								  //the
signum must be inverted ? v1.mul(q2.w); v2.mul(q1.w); w.sub(v2); w.add(v1);
