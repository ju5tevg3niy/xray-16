...
u16 mbhMulti2Wide(xr_wide_char* WideStr,
                  xr_wide_char* WidePos,
                  u16 WideStrSize,
                  const char* MultiStr);

IC bool IsNeedSpaceCharacter(xr_wide_char wc) {
  return ((wc == 0x0020) || (wc == 0x3000) || (wc == 0xFF01) ||
          (wc == 0xFF0C) ||
          // ( wc == 0xFF0D ) ||
          (wc == 0xFF0E) || (wc == 0xFF1A) || (wc == 0xFF1B) ||
          (wc == 0xFF1F) || (wc == 0x2026) || (wc == 0x3002) || (wc == 0x3001));
}
...
IC bool IsBadStartCharacter(xr_wide_char wc) {
  return (IsNeedSpaceCharacter(wc) || (wc == 0x0021) || (wc == 0x002C) ||
          ...
          (wc == 0x003F) || (wc == 0x0029) || (wc == 0xFF09));
}

IC bool IsBadEndCharacter(xr_wide_char wc) {
  return ((wc == 0x0028) || (wc == 0xFF08) || (wc == 0x4E00));
}
