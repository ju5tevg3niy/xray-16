...
ICF void load_function(T& func, cpcstr name) {
  func = reinterpret_cast<T>(GetProcAddress(
      s_dbghelp, name));  // NOLINT(clang-diagnostic-cast-function-type-strict)
...
struct StackTraceBuilder {
  ...
  bool GetNextStackFrameString(LPSTACKFRAME stackFrame,
                               PCONTEXT threadCtx,
                               std::string& frameStr);
...
StackTraceBuilder::StackTraceBuilder() {
  ...
  if (!IsInitialized) {
    Msg("[StackTraceBuilder] SymInitialize failed with error: 0x%u",
        GetLastError());
  }
}
...
bool StackTraceBuilder::GetNextStackFrameString(LPSTACKFRAME stackFrame,
                                                PCONTEXT threadCtx,
                                                std::string& frameStr) {
  BOOL result = stackWalk(MACHINE_TYPE, GetCurrentProcess(), GetCurrentThread(),
                          stackFrame, threadCtx, nullptr,
  ...
  /// Module name
  ///
  HINSTANCE hModule = (HINSTANCE)symGetModuleBase(GetCurrentProcess(),
  ...
  /// Address
  ///
  xr_sprintf(formatBuff, _countof(formatBuff), " at %p",
             stackFrame->AddrPC.Offset);
  frameStr.append(formatBuff);
  ...
  ///
  u8 arrSymBuffer[512]{};
  PIMAGEHLP_SYMBOL functionInfo =
      reinterpret_cast<PIMAGEHLP_SYMBOL>(arrSymBuffer);
  functionInfo->SizeOfStruct = sizeof(*functionInfo);
  functionInfo->MaxNameLength =
      sizeof(arrSymBuffer) - sizeof(*functionInfo) + 1;
  DWORD_PTR dwFunctionOffset;

  result = symGetSymFromAddr(GetCurrentProcess(), stackFrame->AddrPC.Offset,
                             &dwFunctionOffset, functionInfo);

  if (result) {
    if (dwFunctionOffset) {
      xr_sprintf(formatBuff, _countof(formatBuff), " %s() + %Iu byte(s)",
                 functionInfo->Name, dwFunctionOffset);
    } else {
  ...
  result = symGetLineFromAddr(GetCurrentProcess(), stackFrame->AddrPC.Offset,
                              &dwLineOffset, &sourceInfo);

  if (result) {
    if (dwLineOffset) {
      xr_sprintf(formatBuff, _countof(formatBuff),
                 " in %s line %u + %u byte(s)", sourceInfo.FileName,
                 sourceInfo.LineNumber, dwLineOffset);
    } else {
      xr_sprintf(formatBuff, _countof(formatBuff), " in %s line %u",
...
std::vector<std::string> BuildStackTrace(PCONTEXT threadCtx,
                                         u16 maxFramesCount) {
...
#error CPU architecture is not supported.
  ...
  while (builder.GetNextStackFrameString(&stackFrame, threadCtx, frameStr) &&
...
std::vector<std::string> BuildStackTrace(u16 maxFramesCount) {
  ...
  RtlCaptureContext(&currentThreadCtx);  /// GetThreadContext can't be used on
                                         /// the current thread
...
std::vector<std::string> BuildStackTrace(u16 maxFramesCount) {
  ...
  void** array =
      reinterpret_cast<void**>(xr_alloca(sizeof(void*) * maxFramesCount));
  int nptrs = backtrace(
      array, maxFramesCount);  // get void*'s for all entries on the stack
  ...
  if (strings) {
    ...
    for (int i = 1; i < nptrs; i++)  // skip this function
    {
      ...
      if (dladdr(array[i], &info)) {
        if (info.dli_sname) {
          int status = -1;
          demangledName = abi::__cxa_demangle(info.dli_sname, demangledName,
                                              &demangledBufSize, &status);
          if (status == 0) {
