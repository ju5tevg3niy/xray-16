...
uint64_t _cdecl _strtoui64_l(const char* nptr,
                             char** endptr,
                             int base,
                             locale_t locale) {
  BOOL negative = FALSE;
...
CInifile::CInifile(pcstr fileName,
                   bool readOnly,
                   bool loadAtStart,
                   bool saveAtEnd,
                   u32 sect_count,
...
static void insert_item(CInifile::Sect* tgt, const CInifile::Item& I) {
  auto sect_it = std::lower_bound(tgt->Data.begin(), tgt->Data.end(),
                                  I.first.c_str(), item_pred);
...
void CInifile::Load(IReader* F,
...
#ifndef XR_PLATFORM_WINDOWS  // XXX: replace with runtime check for
                             // case-sensitivity
            if (I == nullptr) {
    ...
    } else if (str[0] && str[0] == '[')  // new section ?
    {
      ...
      if (Current) {
        ...
        auto I = std::lower_bound(DATA.begin(), DATA.end(),
                                  Current->Name.c_str(), sect_pred);
        if (I != DATA.end() && (*I)->Name == Current->Name)
          xrDebug::Fatal(DEBUG_INFO, "Duplicate section '%s' found.",
                         Current->Name.c_str());
      ...
      if (nullptr != inherited_names) {
        ...
        for (k = 0; k < cnt; ++k) {
          ...
          _GetItem(inherited_names, k, tmp);
          Sect& inherited_section = r_section(tmp);
          for (auto it = inherited_section.Data.begin();
               it != inherited_section.Data.end(); ++it)
          ...
          *t = 0;
          ...
          if (bInsideSTR)  // multiline str value
          {
            ...
            while (bInsideSTR) {
              ...
              if (!(strlen(value_raw) + strlen(str_add_raw) <
                    sizeof value_raw) ||
                  incorrectFormat) {
                Msg("! Incorrect inifile format: section[%s], variable[%s]. "
  ...
  if (Current) {
    auto I = std::lower_bound(DATA.begin(), DATA.end(), Current->Name.c_str(),
                              sect_pred);
    if (I != DATA.end() && (*I)->Name == Current->Name)
      xrDebug::Fatal(DEBUG_INFO, "Duplicate section '%s' found.",
                     Current->Name.c_str());
...
void CInifile::save_as(IWriter& writer, bool bcheck) const {
  ...
  for (auto r_it = DATA.begin(); r_it != DATA.end(); ++r_it) {
    ...
    for (auto s_it = (*r_it)->Data.begin(); s_it != (*r_it)->Data.end();
         ++s_it) {
      const Item& I = *s_it;
      if (I.first.c_str()) {
        if (I.second.c_str()) {
...
CInifile::Sect& CInifile::r_section(pcstr S) const {
  ...
  if (I == DATA.cend())
  ...
  else if (xr_strcmp((*I)->Name.c_str(), section)) {
    // g_pStringContainer->verify();

    // string_path ini_dump_fn, path;
    // strconcat (sizeof(ini_dump_fn), ini_dump_fn, Core.ApplicationName, "_",
    // Core.UserName, ".ini_log");
    //
    ...
    xrDebug::Fatal(DEBUG_INFO,
                   "Can't open section '%s' (only '%s' avail). Please attach "
                   "[*.ini_log] file to your bug report",
...
bool CInifile::r_line(const shared_str& S, int L, pcstr* N, pcstr* V) const {
...
void CInifile::w_string(pcstr S, pcstr L, pcstr V, pcstr comment) {
  ...
  // #endif
  auto it = std::lower_bound(data.Data.begin(), data.Data.end(),
                             I.first.c_str(), item_pred);
  ...
  if (it != data.Data.end()) {
    ...
    if (0 == xr_strcmp(it->first.c_str(), I.first.c_str())) {
      ...
      R_ASSERT2(b,
                make_string("name[%s] already exist in section[%s]", line, sect)
                    .c_str());
      *it = I;
    } else
...
void CInifile::w_color(pcstr S, pcstr L, u32 V, pcstr comment) {
  string128 temp;
  xr_sprintf(temp, sizeof temp, "%d,%d,%d,%d", color_get_R(V), color_get_G(V),
             color_get_B(V), color_get_A(V));
  w_string(S, L, temp, comment);
}
...
void CInifile::remove_include(cpcstr include) {
  const auto it = std::find_if(m_includes.begin(), m_includes.end(),
...
bool CInifile::try_read(Ivector4& outValue, pcstr section, pcstr line) const {
  ...
  return 4 == sscanf(C, "%d,%d,%d,%d", &outValue.x, &outValue.y, &outValue.z,
                     &outValue.w);
}
