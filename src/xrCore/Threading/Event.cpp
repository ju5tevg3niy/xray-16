...
Event::Event(void* event) noexcept {
  handle = event;
}
...
#elif defined(XR_PLATFORM_POSIX)  // XXX: maybe optimize the implementation or
                                  // even replace with std or SDL
...
void Event::Wait() noexcept {
  ...
  m_id.signaled =
      false;  // due in WaitForSingleObject() "Before returning, a wait function
              // modifies the state of some types of synchronization"
