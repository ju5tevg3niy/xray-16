...
#if defined(XR_PLATFORM_OPENBSD) || \
    (defined(XR_PLATFORM_FREEBSD) && __FreeBSD_version < 1201519)
...
void SetThreadNameImpl(pcstr name) {
  ...
  static auto setThreadDescription = reinterpret_cast<SetThreadDescriptionProc>(
      GetProcAddress(kernelHandle, "SetThreadDescription"));
...
void SetCurrentThreadName(cpcstr name) {
  if (auto error = pthread_setname_np(pthread_self(), name) != 0) {
    Msg("SetCurrentThreadName: failed to set thread name to '%s'. Errno: '%d'",
        name, error);
  }
#ifdef TRACY_ENABLE
  tracy::SetThreadName(name);
