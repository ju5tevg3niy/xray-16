...
#elif defined(XR_ARCHITECTURE_X64) || defined(XR_ARCHITECTURE_ARM64) || \
    defined(XR_ARCHITECTURE_E2K) || defined(XR_ARCHITECTURE_PPC64)
...
// Note: libc++ 8.0 and later define __cpp_lib_hardware_interference_size but
// don't actually implement it
// XXX: check for particular libc++ version, when interference size support will
// be implemented
#if defined(__cpp_lib_hardware_interference_size) && !defined(_LIBCPP_VERSION)
constexpr size_t TASK_SIZE =
    std::max(RECOMMENDED_TASK_SIZE,
...
class Task final : Noncopyable {
 ...
 private:
  ...
  struct Data {
    ...
    Task* parent;
    std::atomic_int16_t
        jobs;  // at least 1 (task itself), zero means task is done.
    std::atomic_bool has_result{};

    Data() = default;
    Data(CallFunc* call, Task* parent) : call(call), parent(parent), jobs(1) {}
  };
  ...
  struct Dispatcher<
      ...
      true,
      std::enable_if_t<
          std::is_same_v<void, std::invoke_result_t<Invokable, Task&>>>> {
  ...
  struct Dispatcher<
      Invokable,
      false,
      std::enable_if_t<std::is_same_v<void, std::invoke_result_t<Invokable>>>> {
    static void Call(Task& task) {
      auto& obj = *reinterpret_cast<Invokable*>(task.m_user_data);
      obj();
      if constexpr (!std::is_trivially_copyable_v<Invokable>)
  ...
  struct Dispatcher<
    ...
    static_assert(sizeof(std::invoke_result_t<Invokable>) <= USER_DATA_SIZE,
                  "Not enough storage to save result of your function. Try to "
                  "reduce its size.");
  ...
  struct Dispatcher<
      ...
      false,
      std::enable_if_t<
          !std::is_same_v<void, std::invoke_result_t<Invokable>>>> {
    static_assert(sizeof(std::invoke_result_t<Invokable>) <= USER_DATA_SIZE,
                  "Not enough storage to save result of your function. Try to "
                  "reduce its size.");
 ...
 private:
  ...
  Task(Invokable func, Task* parent = nullptr)
    ...
    static_assert(sizeof(Invokable) <= USER_DATA_SIZE,
                  "Not enough storage to save your functor/lambda. Try to "
                  "reduce its size.");

    if constexpr (!std::is_empty_v<Invokable> ||
                  !std::is_trivially_copyable_v<Invokable>) {
      ::new (m_user_data) Invokable(std::move(func));
    }

    if (parent) {
      VERIFY2(parent->m_data.jobs.load(std::memory_order_relaxed) > 0,
              "Adding child task to a parent that has already finished.");
      [[maybe_unused]] const auto prev =
          parent->m_data.jobs.fetch_add(1, std::memory_order_acq_rel);
      VERIFY2(prev != std::numeric_limits<decltype(prev)>::max(),
              "Max jobs overflow. (too much children)");
    }
  }
  ...
  void operator()() {
    ...
    for (Task* it = this;; it = it->m_data.parent) {
      const auto unfinishedJobs =
          it->m_data.jobs.fetch_sub(1, std::memory_order_acq_rel) -
          1;  // fetch_sub returns previous value
