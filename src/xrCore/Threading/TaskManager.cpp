...
static constexpr size_t OTHER_THREADS_COUNT =
    1;  // Primary and Secondary thread

class TaskStorageSize {
  template <int E,
            typename Dummy = void>  // XXX: remove Dummy workaround when
                                    // gcc will finally match the standard
  struct PowerOfTwo {
...
static constexpr size_t TASK_STORAGE_SIZE =
    TaskStorageSize::Get<12>();  // 4096 tasks
...
class TaskQueue {
 ...
 public:
  void push(Task* task) noexcept {
    ...
    VERIFY2(task_pos - m_head_pos.load(std::memory_order_acquire) <
                TASK_STORAGE_SIZE,
            "Task queue overflow");
  ...
  Task* pop() noexcept {
    ...
    const bool success = m_head_pos.compare_exchange_weak(
        head_pos, head_pos + 1, std::memory_order_acq_rel);
    if (success) {
  ...
  Task* steal() noexcept { return pop(); }

  size_t size() const noexcept { return m_head_pos - m_tail_pos; }
...
void TaskManager::SpawnThreads() {
  ...
  for (u32 i = 0; i < threads; ++i) {
    workerThreads.emplace_back(Threading::RunThread(
        "Task Worker", &TaskManager::TaskWorkerStart, this));
  }
}
...
void TaskManager::RegisterThisThreadAsWorker() {
  ...
  R_ASSERT2(workers.size() < std::thread::hardware_concurrency(),
            "You must change OTHER_THREADS_COUNT if you want to register more "
            "custom threads.");
...
Task* TaskManager::TryToSteal() const {
  std::uniform_int_distribution<u16> dist{0,
                                          static_cast<u16>(workers.size() - 1)};
...
void TaskManager::Wait(const Task& task,
                       bool updateSystemEvents /*= false*/) const {
  ZoneScoped;
  while (!task.IsFinished()) {
    ExecuteOneTask();
    if (s_tl_worker.id == 0 &&
        (xrDebug::ProcessingFailure() || updateSystemEvents))
...
void TaskManager::GetStats(size_t& allocated,
                           size_t& pushed,
                           size_t& finished) {
  std::lock_guard guard{workersLock};
