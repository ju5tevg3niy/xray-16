...
class TaskRange {
 ...
 public:
  ...
  constexpr TaskRange(T begin, T end)
      : m_begin(begin),
        m_end(end),
        m_grain(size() / (TaskScheduler
                              ? TaskScheduler->GetWorkersCount()
                              : std::thread::hardware_concurrency())) {
    if (m_grain <= 0)
      m_grain = 1;
  }

  TaskRange(T begin, T end, size_t grain)
      : m_begin(begin), m_end(end), m_grain(grain > 0 ? grain : 1) {
 ...
 public:
  constexpr const_iterator begin() const noexcept { return iterator(m_begin); }

  constexpr const_iterator end() const noexcept { return iterator(m_end); }
 ...
 public:
  ...
  constexpr bool is_splittable() const noexcept { return m_grain < size(); }
...
class ParallelFor {
 public:
  static decltype(auto) Run(const Range& range,
                            bool wait,
                            const Function& function) {
    auto& task = TaskManager::AddTask(Functor{range, function});
    if (wait) {
      VERIFY2(TaskScheduler,
              "Task scheduler is not yet created. "
              "You should explicitly state that you know this "
              "by setting 'wait' param to false.");
      if (TaskScheduler)
  ...
  static decltype(auto) Run(Task& parent,
                            const Range& range,
                            bool wait,
                            const Function& function) {
...
template <typename Range, typename Function>
decltype(auto) xr_parallel_for(const Range& range,
                               bool wait,
                               const Function& function) {
...
// User can specify if he wants caller thread to wait on the task finish
template <typename Range, typename Function>
decltype(auto) xr_parallel_for(Task& parent,
                               const Range& range,
                               bool wait,
                               const Function& function) {
  return detail::ParallelFor<Range, Function>::Run(parent, range, wait,
                                                   function);
}
...
decltype(auto) xr_parallel_for(Task& parent,
                               const Range& range,
                               const Function& function) {
  return detail::ParallelFor<Range, Function>::Run(parent, range, true,
                                                   function);
}
