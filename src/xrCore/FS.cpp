...
void* FileDownload(pcstr file_name, const int& file_handle, size_t& file_size) {
  ...
  R_ASSERT3(file_size == static_cast<size_t>(r_bytes),
            "Can't read from file : ", file_name);
...
void FileCompress(pcstr fn, pcstr sign, void* data, size_t size) {
  ...
  int H =
      _open(fn, O_BINARY | O_CREAT | O_WRONLY | O_TRUNC, S_IREAD | S_IWRITE);
  R_ASSERT2(H > 0, fn);
...
CVirtualFileRW::CVirtualFileRW(pcstr cFileName) {
  ...
  R_ASSERT3(hSrcFile != INVALID_HANDLE_VALUE, cFileName,
            xrDebug::ErrorToString(GetLastError()));
  ...
  hSrcFile = ::open(conv_path, O_RDWR | O_CREAT,
                    S_IRUSR | S_IWUSR | S_IRGRP |
                        S_IROTH);  // за такое использование указателя нужно
                                   // убивать, но пока пусть будет
  xr_free(conv_path);
  ...
  data = (char*)::mmap(NULL, Size, PROT_READ | PROT_WRITE, MAP_SHARED, hSrcFile,
                       0);
  R_ASSERT3(data && data != MAP_FAILED, cFileName,
            xrDebug::ErrorToString(GetLastError()));
#else
#error Select or add implementation for your platform
#endif
...
CVirtualFileReader::CVirtualFileReader(pcstr cFileName) {
  ...
  // Open the file
  hSrcFile =
      CreateFile(cFileName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
                 OPEN_EXISTING, 0, 0);
  R_ASSERT3(hSrcFile != INVALID_HANDLE_VALUE, cFileName,
            xrDebug::ErrorToString(GetLastError()));
  ...
  pstr conv_path = xr_strdup(cFileName);
  convert_path_separators(conv_path);
  hSrcFile = ::open(conv_path, O_RDONLY | O_CREAT,
                    S_IRUSR | S_IWUSR | S_IRGRP |
                        S_IROTH);  // за такое использование указателя нужно
                                   // убивать, но пока пусть будет
  xr_free(conv_path);
