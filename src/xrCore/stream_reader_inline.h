...
IC const int& CStreamReader::file_mapping_handle() const {
  return (m_file_mapping_handle);
}
#else
...
#elif defined(XR_PLATFORM_POSIX)
IC void CStreamReader::unmap() {
  ::munmap(const_cast<u8*>(m_current_map_view_of_file), m_current_window_size);
}
#else
#error Select or add implementation for your platform
#endif
...
IC const size_t& CStreamReader::length() const {
  return m_file_size;
}
