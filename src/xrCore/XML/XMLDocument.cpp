...
XMLDocument::XMLDocument()
    : m_xml_file_name(),
      ...
      m_bIgnoreMissingEndTagError(false) {}
...
enum class ParseIncludeResult {
  Success,    /// There is a valid #include and 'out_include_name' returns the
              /// filename
  Error,      /// There is a #include but there is some problem
  NoInclude,  /// There is no #include on this line
};

// Given a string of the form: '#include "filename"' we return the filename in
// 'out_include_name'
ParseIncludeResult ParseInclude(pstr string, pcstr& out_include_name) {
  ...
  // NOTE(Andre): Yes this might look scary but it's perfectly fine. Since the
  // include name is already in the string we are parsing and its not used
  // afterwards we simply replace the closing quote (if there is one) with a
  // null byte and we have a valid c-string pointed to by 'out_include_name' and
  // safe ourselves the need to copy the string.
  *string = '\0';

  return ParseIncludeResult::Success;
}

void ParseFile(pcstr path,
               CMemoryWriter& W,
               IReader* F,
               XMLDocument* xml,
               bool fatal,
  ...
  if (include_depth >= 128) {
    R_ASSERT3(
        !fatal,
        "XML file[%s] parsing failed. Maximum include depth reached (> 128)",
        path);
    Msg("! XML file[%s] parsing failed. Maximum include depth reached (> 128)",
        path);
    return;
  }

  const auto tryOpenFile = [&](IReader*& file, pcstr includeName,
                               pcstr comparePath, pcstr uiPath,
                               pcstr uiPathDelim) {
    if (file)
      return;
    if (includeName == strstr(includeName, comparePath)) {
      shared_str fn =
          xml->correct_file_name(uiPath, strchr(includeName, _DELIMITER) + 1);
      string_path buff;
  ...
  while (!F->eof()) {
    ...
    if (!F->try_r_string(str, sizeof(str))) {
      R_ASSERT3(!fatal,
                "XML file[%s] parsing failed. Line is too long (>= 4096)",
                path);
    ...
    switch (ParseInclude(str, inc_name)) {
      case ParseIncludeResult::Success: {
        ...
        tryOpenFile(I, inc_name, UI_PATH_DEFAULT_WITH_DELIMITER, UI_PATH,
                    UI_PATH_WITH_DELIMITER);
        tryOpenFile(I, inc_name, UI_PATH_DEFAULT_WITH_DELIMITER,
                    UI_PATH_DEFAULT, UI_PATH_DEFAULT_WITH_DELIMITER);
        ...
        if (!I) {
          R_ASSERT4(
              !fatal,
              "XML file[%s] parsing failed. Can't find include file: [%s]",
              path, inc_name);
          Msg("! XML file[%s] parsing failed. Can't find include file: [%s]",
              path, inc_name);
          return;
        }
      ...
      case ParseIncludeResult::Error:
        R_ASSERT4(!fatal, "XML file[%s] invalid include directive: '%s'", path,
                  str);
...
bool XMLDocument::Load(pcstr path_alias,
                       pcstr path,
                       pcstr xml_filename,
                       bool fatal) {
...
bool XMLDocument::Set(pcstr text, bool fatal) {
  ...
  if (m_Doc.Error()) {
    const bool canSkipError =
        IgnoringMissingEndTagError() &&
        m_Doc.ErrorId() == TiXmlBase::TIXML_ERROR_READING_END_TAG;
...
XML_NODE XMLDocument::NavigateToNode(CONST_XML_NODE start_node,
                                     pcstr path,
...
XML_NODE XMLDocument::NavigateToNode(pcstr path,
                                     const size_t node_index) const {
  return NavigateToNode(GetLocalRoot() ? GetLocalRoot() : GetRoot(), path,
                        node_index);
}

XML_NODE XMLDocument::NavigateToNodeWithAttribute(pcstr tag_name,
                                                  pcstr attrib_name,
                                                  pcstr attrib_value) const {
...
pcstr XMLDocument::Read(pcstr path,
                        const size_t index,
                        pcstr default_str_val) const {
  CONST_XML_NODE node = NavigateToNode(path, index);
  pcstr result = Read(node, default_str_val);
  return result;
}

pcstr XMLDocument::Read(CONST_XML_NODE start_node,
                        pcstr path,
                        const size_t index,
                        pcstr default_str_val) const {
...
int XMLDocument::ReadInt(pcstr path,
                         const size_t index,
...
int XMLDocument::ReadInt(CONST_XML_NODE start_node,
                         pcstr path,
                         const size_t index,
                         const int default_int_val) const {
...
float XMLDocument::ReadFlt(pcstr path,
                           const size_t index,
                           float default_flt_val) const {
  pcstr result_str = Read(path, index, nullptr);
  if (result_str == nullptr)
...
float XMLDocument::ReadFlt(CONST_XML_NODE start_node,
                           pcstr path,
                           const size_t index,
                           float default_flt_val) const {
  pcstr result_str = Read(start_node, path, index, nullptr);
...
pcstr XMLDocument::ReadAttrib(CONST_XML_NODE start_node,
                              pcstr path,
                              const size_t index,
                              pcstr attrib,
                              pcstr default_str_val) const {
...
pcstr XMLDocument::ReadAttrib(pcstr path,
                              const size_t index,
                              pcstr attrib,
                              pcstr default_str_val) const {
  CONST_XML_NODE node = NavigateToNode(path, index);
...
pcstr XMLDocument::ReadAttrib(CONST_XML_NODE node,
                              pcstr attrib,
                              pcstr default_str_val) const {
...
int XMLDocument::ReadAttribInt(CONST_XML_NODE node,
                               pcstr attrib,
                               const int default_int_val) const {
...
int XMLDocument::ReadAttribInt(pcstr path,
                               const size_t index,
                               pcstr attrib,
                               int default_int_val) const {
...
int XMLDocument::ReadAttribInt(CONST_XML_NODE start_node,
                               pcstr path,
                               const size_t index,
                               pcstr attrib,
                               const int default_int_val) const {
...
float XMLDocument::ReadAttribFlt(pcstr path,
                                 const size_t index,
                                 pcstr attrib,
...
float XMLDocument::ReadAttribFlt(CONST_XML_NODE start_node,
                                 pcstr path,
                                 const size_t index,
                                 pcstr attrib,
                                 const float default_flt_val) const {
...
float XMLDocument::ReadAttribFlt(CONST_XML_NODE node,
                                 pcstr attrib,
                                 const float default_flt_val) const {
  pcstr result_str = ReadAttrib(node, attrib, nullptr);
...
size_t XMLDocument::GetNodesNum(pcstr path,
                                const size_t index,
                                pcstr tag_name) const {
  CONST_XML_NODE node;
  CONST_XML_NODE root = GetLocalRoot() ? GetLocalRoot() : GetRoot();
...
size_t XMLDocument::GetNodesNum(CONST_XML_NODE node,
                                pcstr tag_name,
                                bool includingComments /*= true*/) const {
  if (!node)
    return 0;
...
XML_NODE XMLDocument::SearchForAttribute(pcstr path,
                                         const size_t index,
                                         pcstr tag_name,
                                         pcstr attrib,
                                         pcstr attrib_value_pattern) const {
...
XML_NODE XMLDocument::SearchForAttribute(CONST_XML_NODE start_node,
                                         pcstr tag_name,
                                         pcstr attrib,
                                         pcstr attrib_value_pattern) const {
  while (start_node) {
...
pcstr XMLDocument::CheckUniqueAttrib(CONST_XML_NODE start_node,
                                     pcstr tag_name,
                                     pcstr attrib_name) {
