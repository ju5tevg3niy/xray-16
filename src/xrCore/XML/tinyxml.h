...
class TiXmlBase {
  ...
  */
  ...
  /** Return the position, in the original source file, of this node or
     attribute. The row and column are 1-based. (That is the first row and first
     column is 1,1). If the returns values are 0 or less, then the parser does
     not have a row and column value.

      Generally, the row and column value will be set when the
     TiXmlDocument::Load(), TiXmlDocument::LoadFile(), or any TiXmlNode::Parse()
     ...
     Computation can be disabled if TiXmlDocument::SetTabSize() is called with 0
     as the value.
 ...
 protected:
  ...
  inline static bool IsWhiteSpace(int c) {
    ...
    return false;  // Again, only truly correct for English/Latin...but usually
                   // works.
  }
...
// static bool StreamTo( std::istream * in, int character, TIXML_STRING * tag );
  ...
  */
  ...
  inline static const char* GetChar(const char* p,
    ...
    if (*length == 1) {
      ...
      *_value = *p;
      ...
      // strncpy( _value, p, *length );	// lots of compilers don't like this
      // function (unsafe), and the null terminator isn't needed
  ...
  // sequences. Ignore case only works for english, and should only be relied on
  // when comparing to English words: StringEqual( p, "version", true ) is fine.
...
class TiXmlNode : public TiXmlBase {
  ...
  */
  ...
  TiXmlNode* FirstChild(const char* _value) {
    ...
    // Exiting syntax: cast this to a const (always safe) call the method, cast
    // the return back to non-const.
  ...
  */
  ...
  /** Accept a hierchical visit the nodes in the TinyXML DOM. Every node in the
     ...
     re-parse the XML for the callbacks, so the performance of TinyXML is
  ...
  /** Convenience function for easy access to the text inside an element.
     ...
     node, it is another element. From this XML:
      ...
      WARNING: GetText() accesses a child node - don't become confused with the
               similarly named TiXmlHandle::Text() and TiXmlNode::ToText() which
...
class TiXmlDocument : public TiXmlNode {
 public:
  ...
  /** Parse the given null terminated block of xml data. Passing in an encoding
     to this method (either TIXML_ENCODING_LEGACY or TIXML_ENCODING_UTF8 will
     force TinyXml to use that encoding, regardless of what TinyXml might
  ...
  */
  ...
  /** Returns the location (if known) of the error. The first column is column
     1, and the first row is row 1. A value of 0 means the row and column wasn't
     applicable (memory errors, for example, have no row/column) or the parser
     lost the error. (An error in the error reporting, in that case.)
