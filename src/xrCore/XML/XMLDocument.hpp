...
class XMLDocument : public Noncopyable {
 public:
  ...
  pcstr Read(CONST_XML_NODE start_node,
             pcstr path,
             const size_t index,
             pcstr default_str_val) const;
  pcstr Read(CONST_XML_NODE node, pcstr default_str_val) const;

  int ReadInt(pcstr path, const size_t index, const int default_int_val) const;
  int ReadInt(CONST_XML_NODE start_node,
              pcstr path,
              const size_t index,
              const int default_int_val) const;
  int ReadInt(CONST_XML_NODE node, const int default_int_val) const;

  float ReadFlt(pcstr path, const size_t index, float default_flt_val) const;
  float ReadFlt(CONST_XML_NODE start_node,
                pcstr path,
                const size_t index,
                float default_flt_val) const;
  float ReadFlt(CONST_XML_NODE node, float default_flt_val) const;

  pcstr ReadAttrib(pcstr path,
                   const size_t index,
                   pcstr attrib,
                   pcstr default_str_val = "") const;
  pcstr ReadAttrib(CONST_XML_NODE start_node,
                   pcstr path,
                   const size_t index,
                   pcstr attrib,
                   pcstr default_str_val = "") const;
  pcstr ReadAttrib(CONST_XML_NODE node,
                   pcstr attrib,
                   pcstr default_str_val) const;

  int ReadAttribInt(pcstr path,
                    const size_t index,
                    pcstr attrib,
                    int default_int_val = 0) const;
  int ReadAttribInt(CONST_XML_NODE start_node,
                    pcstr path,
                    const size_t index,
                    pcstr attrib,
                    const int default_int_val = 0) const;
  int ReadAttribInt(CONST_XML_NODE node,
                    pcstr attrib,
                    const int default_int_val) const;

  float ReadAttribFlt(pcstr path,
                      const size_t index,
                      pcstr attrib,
                      const float default_flt_val = 0.0f) const;
  float ReadAttribFlt(CONST_XML_NODE start_node,
                      pcstr path,
                      const size_t index,
                      pcstr attrib,
                      const float default_flt_val = 0.0f) const;
  float ReadAttribFlt(CONST_XML_NODE node,
                      pcstr attrib,
                      const float default_flt_val = 0.0f) const;

  XML_NODE SearchForAttribute(pcstr path,
                              const size_t index,
                              pcstr tag_name,
                              pcstr attrib,
                              pcstr attrib_value_pattern) const;
  XML_NODE SearchForAttribute(CONST_XML_NODE start_node,
                              pcstr tag_name,
                              pcstr attrib,
                              pcstr attrib_value_pattern) const;
  ...
  size_t GetNodesNum(CONST_XML_NODE node,
                     pcstr tag_name,
  ...
  pcstr CheckUniqueAttrib(CONST_XML_NODE start_node,
                          pcstr tag_name,
                          pcstr attrib_name);
  ...
  XML_NODE NavigateToNode(CONST_XML_NODE start_node,
                          pcstr path,
                          const size_t node_index = 0) const;
  XML_NODE NavigateToNodeWithAttribute(pcstr tag_name,
                                       pcstr attrib_name,
  ...
  bool IgnoringMissingEndTagError() const {
    return m_bIgnoreMissingEndTagError;
  }
  void IgnoreMissingEndTagError(bool value) {
    m_bIgnoreMissingEndTagError = value;
  }
