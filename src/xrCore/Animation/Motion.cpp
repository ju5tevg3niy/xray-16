...
CCustomMotion::CCustomMotion()
    : mtype(), iFrameStart(), iFrameEnd(), fFPS(30.) {}
...
BOOL COMotion::ScaleKeys(float from_time,
                         float to_time,
                         float scale_factor) const {
  BOOL bRes = TRUE;
  for (size_t ch = 0; ch < ctMaxChannel; ch++)
    if (FALSE == (bRes = envs[ch]->ScaleKeys(from_time, to_time, scale_factor,
                                             1.f / fFPS)))
      break;
  return bRes;
}
  ...
  if (vers == 0x0004) {
    ...
    if (vers == 0x0005) {
      ...
      if (vers >= 0x0006) {
        ...
        string64 buf;
        for (auto bm_it = bone_mots.begin(); bm_it != bone_mots.end();
             ++bm_it) {
          F.r_stringZ(buf, sizeof(buf));
          bm_it->SetName(buf);
