...
bool SBoneShape::Valid() const {
  switch (type) {
    case stBox:
      return !fis_zero(box.m_halfsize.x) && !fis_zero(box.m_halfsize.y) &&
...
void SJointIKData::Export(IWriter& F) {
  ...
  for (SJointLimit& joint_limit : limits) {
    ...
    // F.w_float (std::min(-joint_limit.limit.x,-joint_limit.limit.y)); // min
    // (swap special for ODE) F.w_float
    // (std::max(-joint_limit.limit.x,-joint_limit.limit.y)); // max (swap
    // special for ODE)

    VERIFY(std::min(-joint_limit.limit.x, -joint_limit.limit.y) ==
           -joint_limit.limit.y);
    VERIFY(std::max(-joint_limit.limit.x, -joint_limit.limit.y) ==
           -joint_limit.limit.x);
