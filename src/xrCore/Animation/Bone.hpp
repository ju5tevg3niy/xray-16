...
class CBoneInstance {
 ...
 public:
  ...
  [[nodiscard]] IC BOOL callback_overwrite() const {
...
class IBoneData {
 public:
  ...
  [[nodiscard]] virtual u16 GetNumChildren()
      const = 0;  // XXX: maybe change return type to size_t instead of u16.
                  // Check later.
...
class CBone final : public CBoneInstance, public IBoneData {
 ...
 public:
  ...
  [[maybe_unused]] [[nodiscard]] IC const Fmatrix& LTransform() const {
    return mTransform;
  }
  [[maybe_unused]] IC Fmatrix& RenderTransform() {
  ...
  [[nodiscard]] IC float engine_lo_limit(u8 k) const {
    return -IK_data.limits[k].limit.y;
  }
  [[nodiscard]] IC float engine_hi_limit(u8 k) const {
    return -IK_data.limits[k].limit.x;
  }
  [[nodiscard]] IC float editor_lo_limit(u8 k) const {
    return IK_data.limits[k].limit.x;
  }
  [[nodiscard]] IC float editor_hi_limit(u8 k) const {
    return IK_data.limits[k].limit.y;
  }
  ...
  bool ExportOGF(IWriter& F);  // XXX: implementation is missing. Probably it is
                               // in the SDK code?

 private:
  [[nodiscard]] IBoneData& GetChild(u16 id) override { return *children[id]; }
  [[nodiscard]] const IBoneData& GetChild(u16 id) const override {
    return *children[id];
  }
  [[nodiscard]] u16 GetSelfID() const override { return (u16)SelfID; }
  [[nodiscard]] u16 GetNumChildren() const override {
    return u16(children.size());
  }
  [[nodiscard]] const SJointIKData& get_IK_data() const override {
    return IK_data;
  }
  [[nodiscard]] const Fmatrix& get_bind_transform() const override {
  ...
  [[nodiscard]] const Fvector& get_center_of_mass() const override {
  ...
  [[nodiscard]] u16 GetParentID() const override {
    return parent ? u16(parent->SelfID) : u16(-1);
  }
  [[nodiscard]] float lo_limit(u8 k) const override {
    return engine_lo_limit(k);
  }
  [[nodiscard]] float hi_limit(u8 k) const override {
    return engine_hi_limit(k);
  }
};
