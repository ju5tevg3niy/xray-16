...
BOOL motions_value::load(pcstr N, IReader* data, vecBones* bones) {
  ...
  if (MP) {
    ...
    for (u16 part_i = 0; part_i < part_count; part_i++) {
      ...
      for (std::vector<u32>::iterator b_it = PART.bones.begin();
    ...
    if (part_bone_cnt != (u16)bones->size()) {
      bRes = false;
      Msg("! Different bone count[%s] [Object: '%d' <-> Motions: '%d']", N,
          bones->size(), part_bone_cnt);
    }
#else
    VERIFY3(part_bone_cnt == (u16)bones->size(), "Different bone count '%s'",
            N);
#endif
    if (bRes) {
    ...
    xrDebug::Fatal(DEBUG_INFO, "Old skinned model version unsupported! (%s)",
                   N);
  }
  if (!bRes)
    return false;
