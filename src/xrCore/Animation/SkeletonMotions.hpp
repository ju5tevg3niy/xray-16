...
using BoneMotionMap = std::map<shared_str, MotionVec>;
...
struct motions_value {
  ...
  u32 mem_usage() {
    ...
    for (auto bm_it = m_motions.begin(); bm_it != m_motions.end(); ++bm_it)
      for (auto m_it = bm_it->second.begin(); m_it != bm_it->second.end();
           ++m_it)
        sz += m_it->mem_usage();
    return sz;
  }
...
class shared_motions {
 ...
 public:
  bool create(shared_str key,
              IReader* data,
              vecBones* bones);  //{ motions_value* v =
  // g_pMotionsContainer->dock(key,data,bones); if (0!=v)
  // v->m_dwReference++; destroy(); p_ = v; }
  bool create(
      shared_motions const& rhs);  // { motions_value* v = rhs.p_; if (0!=v)
                                   // v->m_dwReference++; destroy(); p_ = v; }
 public:
  // construction
  shared_motions() { p_ = 0; }
