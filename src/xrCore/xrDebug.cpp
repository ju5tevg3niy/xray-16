...
AssertionResult xrDebug::ShowMessage(pcstr title,
                                     pcstr message,
                                     bool simpleMode) {
  ...
  switch (result) {
    case IDCANCEL:
      return AssertionResult::abort;
    case IDTRYAGAIN:
      return AssertionResult::tryAgain;
    case IDCONTINUE:
...
SDL_AssertState SDLAssertionHandler(const SDL_AssertData* data,
  ...
  const auto result = xrDebug::Fail(
      alwaysIgnore, {data->filename, data->linenum, data->function},
      data->condition, desc);
...
void xrDebug::SetBugReportFile(const char* fileName) {
  xr_strcpy(BugReportFile, fileName);
}
...
void xrDebug::GatherInfo(char* assertionInfo,
                         size_t bufferSize,
...
AssertionResult xrDebug::Fail(bool& ignoreAlways,
                              const ErrorLocation& loc,
                              const char* expr,
                              long hresult,
                              const char* arg1,
                              const char* arg2) {
  return Fail(ignoreAlways, loc, expr, xrDebug::ErrorToString(hresult), arg1,
              arg2);
}
...
AssertionResult xrDebug::Fail(bool& ignoreAlways,
  ...
  if (windowHandler)
    windowHandler->OnErrorDialog(
        true);  // Call it only after locking so that multiple threads won't
  ...
  if (ShowErrorMessage) {
    xr_strcat(assertionInfo,
              ...
              "Press CONTINUE to continue execution and ignore all the errors "
              "of this type\r\n"
              "\r\n");
  }
  ...
  if (Core.PluginMode)
    /*result =*/ShowMessage("X-Ray error",
                            assertionInfo);  // Do not assign 'result'
  else {
    if (ShowErrorMessage)
  ...
  if (resetFullscreen)
    windowHandler->OnErrorDialog(
        false);  // Call it only before unlocking so that multiple threads won't
                 // call this function simultaneously.
...
void xrDebug::DoExit(const std::string& message) {
  ...
  if (ShowErrorMessage) {
    const auto result =
        ShowMessage(Core.ApplicationName, message.c_str(), false);
...
pcstr xrDebug::ErrorToString(long code) {
  ...
  if (!result) {
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, code, 0, descStorage,
                  sizeof(descStorage) - 1, 0);
    result = descStorage;
  }
#endif
  return result;
...
int out_of_memory_handler(size_t size) {
  ...
  if (cb)
    ...
    Memory.mem_compact();
    const auto [ecoStringsBytes, ecoStringsCount] =
        g_pStringContainer->stat_economy();
    const size_t ecoSmem = g_pSharedMemoryContainer->stat_economy();
    Msg("* [x-ray]: shared strings: memory[%ld K], count[%lu]",
        ecoStringsBytes / 1024, ecoStringsCount);
    Msg("* [x-ray]: shared memory[%ld K]", ecoSmem);
  }
  xrDebug::Fatal(DEBUG_INFO, "Out of memory. Memory request: %zu K",
                 size / 1024);
  return 1;
}
...
void WINAPI xrDebug::PreErrorHandler(INT_PTR) {
  ...
  if (xr_FS && FS.m_Flags.test(CLocatorAPI::flReady)) {
    ...
    __try {
      ...
      // XXX: Refactor Console to accept user config filename on initialization
      // or even construction!
      // XXX: Maybe refactor CCC_LoadCFG, move code for loading user.ltx into a
      // generic function
      const auto cfg_name = userConfigHandler
                                ? userConfigHandler->GetUserConfigFileName()
                                : "user.ltx";
...
LONG WINAPI xrDebug::UnhandledFilter(EXCEPTION_POINTERS* exPtrs) {
  ...
  if (!ErrorAfterDialog && !strstr(GetCommandLine(), "-no_call_stack_assert")) {
    ...
    std::vector<std::string> stackTrace =
        BuildStackTrace(exPtrs->ContextRecord, 1024);
  ...
  if (msgRes != AssertionResult::abort || reportRes == frrvLaunchDebugger) {
    constexpr cpcstr debugger =
        "Please, attach the debugger to the process"
...
void xr_terminate() {
  ...
  string4096 assertionInfo;
  xrDebug::GatherInfo(assertionInfo, sizeof(assertionInfo), DEBUG_INFO, nullptr,
...
static void invalid_parameter_handler(const wchar_t* expression,
  ...
  if (expression)
    wcstombs_s(&convertedChars, mbExpression, sizeof(mbExpression), expression,
               (wcslen(expression) + 1) * 2);
  else
    xr_strcpy(mbExpression, "");
  if (function)
    wcstombs_s(&convertedChars, mbFunction, sizeof(mbFunction), function,
               (wcslen(function) + 1) * 2);
  else
    xr_strcpy(mbFunction, __FUNCTION__);
  if (file)
    wcstombs_s(&convertedChars, mbFile, sizeof(mbFile), file,
               (wcslen(file) + 1) * 2);
  else {
    line = __LINE__;
    xr_strcpy(mbFile, __FILE__);
  }
  xrDebug::Fail(ignoreAlways, {mbFile, int(line), mbFunction}, mbExpression,
                "invalid parameter");
}
#endif
...
void xrDebug::OnThreadSpawn() {
  ...
  std::signal(
      SIGFPE, +[](int signal) { handler_base("floating point error"); });
#ifdef DEBUG
  std::signal(SIGSEGV, +[](int signal) { handler_base("segmentation fault"); });
#endif
  std::signal(
      SIGABRT, +[](int signal) { handler_base("application is aborting"); });
  std::signal(
      SIGTERM,
      +[](int signal) { handler_base("termination with exit code 3"); });

#if defined(XR_PLATFORM_WINDOWS)
  std::signal(
      SIGABRT_COMPAT,
      +[](int signal) { handler_base("application is aborting"); });
...
#ifdef MASTER_GOLD
  ShowErrorMessage =
      commandLine ? !!strstr(commandLine, "-show_error_window") : false;
#endif
}
