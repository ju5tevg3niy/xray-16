...
std::string EFS_Utils::ChangeFileExt(const std::string& src, pcstr ext) {
...
bool EFS_Utils::GetOpenNameInternal(pcstr initial,
                                    pstr buffer,
                                    size_t sz_buf,
                                    bool bMulti /*= false*/,
                                    pcstr offset /*= 0*/,
...
bool EFS_Utils::GetSaveName(pcstr initial,
                            string_path& buffer,
                            pcstr offset,
                            int start_flt_ext) {
  // unsigned int dwVersion = GetVersion();
...
//----------------------------------------------------
pcstr EFS_Utils::AppendFolderToName(pstr tex_name,
                                    size_t const tex_name_size,
                                    int depth,
                                    BOOL full_name) {
  string256 _fn;
  xr_strcpy(tex_name, tex_name_size,
            AppendFolderToName(tex_name, _fn, sizeof(_fn), depth, full_name));
  return tex_name;
}

pcstr EFS_Utils::AppendFolderToName(pcstr src_name,
                                    pstr dest_name,
                                    size_t const dest_name_size,
                                    int depth,
                                    BOOL full_name) {
...
pcstr EFS_Utils::GenerateName(pcstr base_path,
                              pcstr base_name,
                              pcstr def_ext,
                              pstr out_name,
                              size_t const out_name_size) {
  int cnt = 0;
  string_path fn;
  if (base_name)
  ...
  while (FS.exist(fn)) {
    if (base_name)
      xr_sprintf(fn, sizeof(fn), "%s%s%02d%s", base_path, base_name, cnt++,
                 def_ext);
    else
      xr_sprintf(fn, sizeof(fn), "%s%02d%s", base_path, cnt++, def_ext);
  }
