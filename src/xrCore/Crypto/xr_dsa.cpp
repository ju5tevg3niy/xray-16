...
shared_str xr_dsa::sign(private_key_t const& priv_key,
                        u8 const* data,
                        u32 const data_size) const {
#ifdef USE_OPENSSL
  BIGNUM* temp{};
  ...
  return ret;
#else
  Msg("! [%s] Engine was built without OpenSSL, verifying will always fail.",
      __FUNCTION__);
  return "";
#endif
}
...
bool xr_dsa::verify(public_key_t const& pub_key,
  ...
  const bool ret =
      EVP_PKEY_verify(m_context, sig_buff, sig_size, data, data_size) == 1;
  return ret;
#else
  Msg("! [%s] Engine was built without OpenSSL, verifying will always fail.",
      __FUNCTION__);
