...
xr_sha1::hash_t xr_sha1::calculate_with_yielder(const u8* data,
                                                u32 data_size,
                                                const yielder_t& yielder) {
#ifdef USE_OPENSSL
  R_ASSERT1_CURE(data && data_size, { return {}; });

  EVP_MD_CTX* sha_ctx = EVP_MD_CTX_new();
  R_ASSERT1_CURE(sha_ctx, { return {}; });
  ...
  [[maybe_unused]]
  const bool success =
      EVP_DigestFinal_ex(sha_ctx, result.data(), &hash_len) != 0;
  VERIFY(success);
