...
xr_dsa_verifyer::xr_dsa_verifyer(
  ...
  static_assert(
      ...
      "Public key sizes not equal.");
  CopyMemory(m_public_key.m_value, public_key, sizeof(m_public_key.m_value));
}

xr_dsa_verifyer::~xr_dsa_verifyer() {}
std::optional<crypto::xr_sha1::hash_t> xr_dsa_verifyer::verify(
    u8 const* data,
    u32 data_size,
...
#endif
  const bool success = m_dsa.verify(m_public_key, hash.data(),
                                    crypto::xr_sha1::DigestSize, dsign);
