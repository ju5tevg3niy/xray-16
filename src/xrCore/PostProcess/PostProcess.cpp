...
void BasicPostProcessAnimator::Load(pcstr name, bool internalFs /*= true*/) {
  ...
  if (ext) {
    if (!xr_strcmp(ext, POSTPROCESS_FILE_EXTENSION)) {
      ...
      FATAL(
          "ERROR: Can't support files with many animations set. Incorrect "
          "file.");
  }
...
void BasicPostProcessAnimator::Create() {
  ...
  m_Params[0] =
      xr_new<CPostProcessColor>(&m_EffectorParams.color_base);  // base color
  VERIFY(m_Params[0]);
  m_Params[1] =
      xr_new<CPostProcessColor>(&m_EffectorParams.color_add);  // add color
  VERIFY(m_Params[1]);
  m_Params[2] =
      xr_new<CPostProcessColor>(&m_EffectorParams.color_gray);  // gray color
  VERIFY(m_Params[2]);
  m_Params[3] =
