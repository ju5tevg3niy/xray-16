...
void Log(pcstr msg, const Fmatrix& dop) {
  const u32 buffer_size =
      (strlen(msg) + 2 + 4 * (4 * (64 + 1) + 1) + 1) * sizeof(char);
  pstr buf = static_cast<pstr>(xr_alloca(buffer_size));

  xr_sprintf(buf, buffer_size,
             "%s:\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n", msg,
             dop.i.x, dop.i.y, dop.i.z, dop._14_, dop.j.x, dop.j.y, dop.j.z,
             dop._24_, dop.k.x, dop.k.y, dop.k.z, dop._34_, dop.c.x, dop.c.y,
             dop.c.z, dop._44_);
  Log(buf);
}
...
void CreateLog(bool nl) {
  ...
  if (unique_logs) {
    ...
    std::strftime(TimeBuf, sizeof(TimeBuf), "%d-%m-%y_%H-%M-%S",
                  std::localtime(&time));
    strconcat(log_file_name, Core.ApplicationName, "_", Core.UserName, "_",
              TimeBuf, ".log");
  } else {
  ...
  if (!unique_logs) {
    ...
    const std::string backup_logFName =
        EFS.ChangeFileExt(log_file_name, ".bkp");
