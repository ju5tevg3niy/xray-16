...
void CStreamReader::construct(const HANDLE& file_mapping_handle,
  ...
  m_window_size =
      std::max(window_size, static_cast<size_t>(FS.dwAllocGranularity));

  map(0);
}
#elif defined(XR_PLATFORM_POSIX)
void CStreamReader::construct(int file_mapping_handle,
                              const size_t& start_offset,
                              const size_t& file_size,
                              const size_t& archive_size,
                              const size_t& window_size) {
...
void CStreamReader::map(const size_t& new_offset) {
  ...
  m_current_map_view_of_file =
      static_cast<u8*>(MapViewOfFile(m_file_mapping_handle, FILE_MAP_READ, 0,
                                     start_offset, m_current_window_size));
#elif defined(XR_PLATFORM_POSIX)
  m_current_map_view_of_file =
      static_cast<u8*>(::mmap(NULL, m_current_window_size, PROT_READ,
                              MAP_SHARED, m_file_mapping_handle, start_offset));
#else
#error Select or add implementation for your platform
#endif
...
void CStreamReader::r_stringZ(shared_str& dest) {
  ...
  do {
    ...
    const size_t current_chunk_size =
        static_cast<size_t>(end_ptr - m_current_pointer);
