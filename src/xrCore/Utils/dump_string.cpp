...
std::string get_string(const Fmatrix& dop) {
  return make_string("\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n%f,%f,%f,%f\n",
                     ...
                     dop.j.z, dop._24_, dop.k.x, dop.k.y, dop.k.z, dop._34_,
                     dop.c.x, dop.c.y, dop.c.z, dop._44_);
}

std::string get_string(const Fbox& box) {
  return make_string("[ min: %s - max: %s ]", get_string(box.vMin).c_str(),
                     get_string(box.vMax).c_str());
}
...
void dump(const char* name, const Fmatrix& form) {
  ...
  // form.i ).c_str( ) , form._14_ ); Msg( "%s, _24_=%f ", dump_string(
  // make_string( "%s.j, ", name ).c_str(), form.j ).c_str( ) , form._24_ );
  // Msg( "%s, _34_=%f ", dump_string( make_string( "%s.k, ", name ).c_str(),
  // form.k ).c_str( ) , form._34_ ); Msg( "%s, _44_=%f ", dump_string(
