...
class xr_jpeg_error_mgr final : public jpeg_error_mgr {
 ...
 public:
  ...
  auto get_jmp_buffer() { return setjmp_buffer; }
...
class xr_jpeg_destination_mgr final : public jpeg_destination_mgr {
 ...
 public:
  xr_jpeg_destination_mgr(jpeg_compress_struct& info, IWriter& writer)
      : m_writer(writer) {
...
bool Image::OpenJPEG(const IReader& reader) {
  return OpenJPEG(static_cast<u8*>(reader.pointer()),
                  static_cast<unsigned long>(reader.elapsed()));
}
...
bool Image::OpenJPEG(const u8* dataPtr, u32 dataSize) {
  ...
  jpeg_create_decompress(&info);
  {
    ...
    switch (channelCount) {
      case 3:
        format = ImageDataFormat::RGB8;
        break;
      case 4:
        format = ImageDataFormat::RGBA8;
        break;
    }
    ...
    jpeg_start_decompress(&info);
    {
      ...
      while (info.output_scanline < info.output_height) {
        buf[0] = JSAMPROW(&((
            u8*)data)[channelCount * info.output_width * info.output_scanline]);
  ...
  xr_jpeg_destination_mgr cdest(info, writer);
  {
    ...
    jpeg_start_compress(&info, TRUE);
    {
      ...
      const auto row_stride =
          width * channelCount; /* JSAMPLEs per row in image_buffer */

      while (info.next_scanline < info.image_height) {
        const auto scanline = invert
                                  ? info.image_height - 1 - info.next_scanline
                                  : info.next_scanline;
