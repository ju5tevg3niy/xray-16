...
void process(u32 const index, u32 const count, pcstr* strings) {
  ...
  pstr temp =
      (pstr)xr_alloca((count * (max_string_size + 4) + 1) * sizeof(**strings));
  pstr k = temp;
  *k++ = '[';
...
template <u32 count>
static inline void process(pstr& i,
                           pcstr e,
                           u32 const index,
                           pcstr (&strings)[count]) {
  VERIFY(i <= e);
...
void check_stack_overflow(u32 stack_increment) {
  ...
  __try {
    ...
    p;
      ...
      xray::core::detail::stack_overflow_exception_filter(GetExceptionCode())) {
    _resetstkoflw();
  }
#endif
}
