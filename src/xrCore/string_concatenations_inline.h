...
class string_tupples {
  ...
  void process_args(const std::tuple<Args...>& args,
 ...
 private:
  ...
  template <u32 index>
  struct helper {
    static size_t get_length(pcstr string) {
    ...
    static size_t get_length(std::string const& string) {
      return string.size();
    }
    ...
    static void add_string(string_tupples& self, TType p) {
      static_assert(index < MAX_ITEM_COUNT,
                    "Error invalid string index specified.");
