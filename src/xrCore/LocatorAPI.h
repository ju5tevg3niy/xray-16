...
#define _A_SUBDIR 0x00000010
    ...
    defined(XR_ARCHITECTURE_PPC64)
#define _finddata_t _finddata64i32_t
#elif defined(XR_ARCHITECTURE_X86) || defined(XR_ARCHITECTURE_ARM) || \
    defined(XR_ARCHITECTURE_ARM64) || defined(XR_ARCHITECTURE_PPC)
...
#error Select or add implementation for your platform
  ...
  struct archive_file_header {
    ...
    static constexpr auto ELEMENTS_SIZE =
        sizeof(size_real) + sizeof(size_compr) + sizeof(crc) + sizeof(ptr);

    archive_file_header(IReader& reader);
    archive_file_header(IWriter& writer,
                        pcstr file_name,
                        u32 real_size,
                        u32 compressed_size,
                        u32 crc_sum,
 ...
 private:
  ...
  const file* Register(pcstr name,
                       size_t vfs,
                       u32 crc,
                       u32 ptr,
                       u32 size_real,
 ...
 private:
  void check_cached_files(pstr fname,
  ...
  bool check_for_file(pcstr path,
                      pcstr _fname,
                      string_path& fname,
                      const file*& desc);
 ...
 public:
  ...
  void _initialize(u32 flags,
                   pcstr target_folder = nullptr,
                   pcstr fs_name = nullptr);
  void _destroy();
  ...
  FileStatus exist(string_path& fn,
                   pcstr path,
                   pcstr name,
  ...
  bool dir_delete(pcstr full_path, bool remove_files) {
    return dir_delete(nullptr, full_path, remove_files);
  }
  ...
  std::vector<pstr>* file_list_open(pcstr initial,
                                    pcstr folder,
  ...
  pcstr update_path(string_path& dest,
                    pcstr initial,
                    pcstr src,
                    bool crashOnNotFound = true);

  size_t file_list(FS_FileSet& dest,
                   pcstr path,
                   u32 flags = FS_ListFiles,
                   pcstr mask = nullptr);
  ...
  void auth_generate(std::vector<shared_str>& ignore,
