...
struct eq_fname_free {
  ...
  bool operator()(_open_file& itm) const {
...
void _check_open_file(const shared_str& _fname) {
  auto it = std::find_if(g_open_files.begin(), g_open_files.end(),
                         eq_fname_check(_fname));
...
const CLocatorAPI::file* CLocatorAPI::RegisterExternal(pcstr name) {
  ...
  if (stat(name, &buffer) == -1)
    return nullptr;
  return Register(name, size_t(-1), 0, 0, buffer.st_size, buffer.st_size,
                  u32(buffer.st_mtime));
}

const CLocatorAPI::file* CLocatorAPI::Register(pcstr name,
                                               size_t vfs,
                                               u32 crc,
                                               u32 ptr,
                                               u32 size_real,
  ...
  while (temp[0]) {
    ...
    if (!exist(path)) {
      ...
      cpcstr failureDescription =
          "Failed to register file in filesystem.\n"
  ...
  while (true) {
    ...
    if ((dwType & ~CFS_CompressMark) == ID) {
      ...
      if (dwType & CFS_CompressMark) {
        ...
        bool result =
            _decompressLZ(&dest, &dest_sz, src_data, dwSize, archiveSize);
...
void CLocatorAPI::LoadArchive(archive& A, pcstr entrypoint) {
  ...
  if (A.header) {
    ...
    if (0 == xr_stricmp(read_path.c_str(), "gamedata")) {
      ...
      xr_strcat(fs_entry_point, sizeof fs_entry_point,
                read_path.c_str() + strlen(alias_name) + 1);
    }
  } else {
  ...
  A.open();
  IReader* hdr =
      open_chunk(A.hSrcFile, 1, A.path.c_str(), A.size, shouldDecrypt);
  ...
  while (!hdr->eof()) {
    ...
    Register(full, A.vfs_idx, header.crc, header.ptr, header.size_real,
             header.size_compr, A.modif);
  }
  hdr->close();
}  //-V773
...
void CLocatorAPI::ProcessArchive(pcstr _path) {
  ...
  IReader* hdr =
      open_chunk(A.hSrcFile, CFS_HeaderChunkID, A.path.c_str(), A.size);
...
#define ENDS_WITH(n) \
  (len > sizeof(n) && !strcmp(_name + len - (sizeof(n) - 1), n))
...
#error Specify code path for your platform
  ...
  if (entry.attrib & _A_SUBDIR) {
    ...
    Register(N, VFS_STANDARD_FILE, 0, 0, entry.size, entry.size,
...
bool ignore_path(pcstr _path) {
#if defined(XR_PLATFORM_WINDOWS)
  HANDLE h =
      CreateFile(_path, 0, 0, nullptr, OPEN_EXISTING,
  ...
  convert_path_separators(conv_path);
  int h = ::open(conv_path, O_RDONLY | O_TRUNC,
...
#elif defined(XR_PLATFORM_POSIX)
      xr_strcpy(fullPath, sizeof fullPath,
                findData.name);  // glob return full path to file
...
#error Select or add implementation for your platform
  ...
  if (newSize > oldSize) {
    std::sort(rec_files.begin() + oldSize, rec_files.end(),
              [](const _finddata_t& x, const _finddata_t& y) {
                return xr_strcmp(x.name, y.name) < 0;
              });
    for (size_t i = oldSize; i < newSize;
         i++)  // Don't replace this with range-based for!
      ProcessOne(path,
                 rec_files[i]);  // only index-based for can work correctly here
...
void CLocatorAPI::setup_fs_path(pcstr fs_name) {
  ...
  if (SDL_strlen(fs_path) != 0) {
    ...
    CHECK_OR_EXIT(tmp_path && tmp_path[0],
                  make_string("Cannot get realpath for \"%s\": %s", fs_path,
                              strerror(errno)));
    SDL_strlcpy(full_current_directory, tmp_path,
                sizeof full_current_directory);
    free(tmp_path);
  } else {
    // If the fs ltx exists in the current working directory use it, if not use
    // the perf path
    if (access(FSLTX, F_OK) == 0)
      ...
      if (strstr(Core.Params, "-shoc") || strstr(Core.Params, "-soc"))
        pref_path = SDL_GetPrefPath("GSC Game World",
                                    "S.T.A.L.K.E.R. - Shadow of Chernobyl");
      else if (strstr(Core.Params, "-cs"))
        pref_path =
            SDL_GetPrefPath("GSC Game World", "S.T.A.L.K.E.R. - Clear Sky");
      else
        pref_path = SDL_GetPrefPath("GSC Game World",
                                    "S.T.A.L.K.E.R. - Call of Pripyat");

      /* A final decision must be made regarding the changed resources. Since
       * only OpenGL shaders remain mandatory for Linux for the entire trilogy,
       * I propose adding shaders from
       * <CMAKE_INSTALL_FULL_DATAROOTDIR>/openxray/gamedata/shaders so that we
       * remove unnecessary questions from users who want to start the game
       * using resources not from the proposed ~/.local/share/GSC Game
       * World/Game in this case, this section of code can be safely removed */
      chdir(pref_path);
       ...
       * If it doesn't, additionally check if not following symlinks is
       * successful (catches symlink itself being broken),
       * -> delete the symlink if it is broken.
       * Then, make a new symlink to our resources.
       * TODO This doesn't account for other stat errors (EACCES, ENAMETOOLONG,
       * ENOENT caused by missing path component) */
      ...
      SDL_strlcpy(full_current_directory, pref_path,
                  sizeof full_current_directory);
      SDL_free(pref_path);
    }
  }
...
void CLocatorAPI::_initialize(u32 flags, pcstr target_folder, pcstr fs_name) {
  ...
  if (m_Flags.is(flTargetFolderOnly)) {
    ...
    while (!pFSltx->eof()) {
      ...
      FS_Path* P =
          xr_new<FS_Path>(p_it != m_paths.end() ? p_it->second->m_Path : root,
...
FileStatus CLocatorAPI::exist(pcstr path,
                              pcstr name,
...
FileStatus CLocatorAPI::exist(string_path& fn,
                              pcstr path,
                              pcstr name,
                              FSType fsType /*= FSType::Virtual*/) {
...
std::vector<pstr>* CLocatorAPI::file_list_open(pcstr initial,
                                               pcstr folder,
                                               u32 flags) {
  string_path N;
...
size_t CLocatorAPI::file_list(FS_FileSet& dest,
                              pcstr path,
...
void CLocatorAPI::check_cached_files(pstr fname,
  ...
  if (bCopy) {
    ...
    Register(fname_in_cache, VFS_STANDARD_FILE, 0, 0, desc.size_real,
...
void CLocatorAPI::file_from_archive(IReader*& R,
  ...
  VERIFY3(ptr && ptr != MAP_FAILED, "cannot create file mapping on file",
          fname);
#else
#error Select or add implementation for your platform
#endif
...
void CLocatorAPI::file_from_archive(CStreamReader*& R,
  ...
  R_ASSERT2(desc.size_compressed == desc.size_real,
            make_string("cannot use stream reading for compressed data %s, do "
                        "not compress data to be streamed",
                        fname));
...
#elif defined(XR_PLATFORM_POSIX)
  R->construct(A.hSrcFile, desc.ptr, desc.size_compressed, A.size,
               BIG_FILE_READER_WINDOW_SIZE);
#else
#error Select or add implementation for your platform
#endif
...
bool CLocatorAPI::check_for_file(pcstr path,
                                 pcstr _fname,
                                 string_path& fname,
                                 const file*& desc) {
  check_pathes();
...
CStreamReader* CLocatorAPI::rs_open(pcstr path, pcstr _fname) {
  return r_open_impl<CStreamReader>(path, _fname);
}
IReader* CLocatorAPI::r_open(pcstr path, pcstr _fname) {
...
void CLocatorAPI::w_close(IWriter*& S) {
  if (S) {
    ...
    if (bReg) {
      ...
      struct _stat st;
      _stat(fname, &st);
      Register(fname, VFS_STANDARD_FILE, 0, 0, st.st_size, st.st_size,
               (u32)st.st_mtime);
#elif defined(XR_PLATFORM_POSIX)
      struct stat st;
      ::stat(fname, &st);
...
FS_Path* CLocatorAPI::append_path(pcstr path_alias,
                                  pcstr root,
                                  pcstr add,
                                  bool recursive) {
  VERIFY(root /**&&root[0]**/);
...
pcstr CLocatorAPI::update_path(string_path& dest,
                               pcstr initial,
                               pcstr src,
...
bool CLocatorAPI::can_write_to_folder(pcstr path) {
  if (path && path[0]) {
    ...
    strconcat(sizeof temp, temp, path,
              path[strlen(path) - 1] != _DELIMITER ? DELIMITER : "", fn);
