...
void xrCore::CalculateBuildId() {
  ...
  const char* monthId[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
void xrCore::PrintBuildInfo() {
  Msg("%s %s build %d, %s (%s)", ApplicationName, XRAY_BUILD_CONFIGURATION,
      buildId, buildDate, XRAY_BUILD_CONFIGURATION2);
...
void xrCore::Initialize(pcstr _ApplicationName,
                        pcstr commandLine,
                        bool init_fs,
                        pcstr fs_fname,
                        bool plugin) {
  ...
  if (0 == init_counter) {
    ...
    CoInitializeEx(nullptr,
                   COINIT_MULTITHREADED);  // needed for OpenAL initialization
    ...
    if (!base_path) {
      if (strstr(Core.Params, "-shoc") || strstr(Core.Params, "-soc"))
        base_path = SDL_GetPrefPath("GSC Game World",
...
void SDLLogOutput(void* /*userdata*/,
  ...
  const size_t size = sizeof(mark) + sizeof(from) + sizeof(type) +
                      sizeof(format) + sizeof(message);
