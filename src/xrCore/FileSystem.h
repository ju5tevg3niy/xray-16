...
class EFS_Utils {
 protected:
  bool GetOpenNameInternal(pcstr initial,
                           pstr buffer,
                           size_t sz_buf,
                           bool bMulti = false,
                           pcstr offset = 0,
 ...
 public:
  void _initialize() {}
  void _destroy() {}
  pcstr GenerateName(pcstr base_path,
                     pcstr base_name,
                     pcstr def_ext,
                     pstr out_name,
                     size_t const out_name_size);

  bool GetOpenName(pcstr initial,
                   string_path& buffer,
                   int sz_buf,
                   bool bMulti = false,
                   pcstr offset = 0,
  ...
  bool GetSaveName(pcstr initial,
                   string_path& buffer,
                   pcstr offset = 0,
                   int start_flt_ext = -1);
  bool GetSaveName(pcstr initial,
  ...
  std::string AppendFolderToName(std::string& tex_name,
                                 int depth,
                                 BOOL full_name);

  pcstr AppendFolderToName(pstr tex_name,
                           size_t const tex_name_size,
                           int depth,
                           BOOL full_name);
  pcstr AppendFolderToName(pcstr src_name,
                           pstr dest_name,
