...
struct PPM_CONTEXT {         // Notes:
  ...
  struct STATE {             // 2. sizeof(BYTE) > sizeof(u8)
...
#define P_CALL(F) \
  (PrefetchData(p->Successor), p->Successor = p->Successor->F(Order + 1))
...
static const u8 ExpEscape[16] = {25, 14, 9, 7, 5, 5, 4, 4,
                                 4,  3,  3, 3, 2, 2, 2, 2};
...
inline SEE2_CONTEXT* PPM_CONTEXT::makeEscFreq2() const {
  ...
  if (NumStats != 0xFF) {
    t = Suffix->NumStats;
    psee2c =
        SEE2Cont[QTable[NumStats + 2] - 3] + (SummFreq > 11 * (NumStats + 1));
...
static void _STDCALL StartModelRare(int MaxOrder, MR_METHOD MRMethod) {
  ...
  if (first_time) {
    ...
    static const u16 InitBinEsc[] = {0x3CDD, 0x1F3F, 0x59BF, 0x48F3,
