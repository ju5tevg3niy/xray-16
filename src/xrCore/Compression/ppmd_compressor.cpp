...
static xr_unique_ptr<Lock>
    PPMD_Lock;  // XXX: can cause crashes on launch/exit due to alloc/free
...
u32 ppmd_compress_mt(void* dest_buffer,
  ...
  while (rest_src_buff) {
    u32 to_compress = rest_src_buff > compress_chunk_size ? compress_chunk_size
                                                          : rest_src_buff;
