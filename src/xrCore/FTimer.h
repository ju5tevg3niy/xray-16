...
class CTimerBase {
 ...
 public:
  constexpr CTimerBase() noexcept
      : startTime(), pauseDuration(), pauseAccum(), paused(false) {}
...
class CStatTimer {
 ...
 public:
  ...
  void AppendResults(Lock& lock, const CStatTimer& other)  // thread-safe
  {
    ...
    VERIFY2(fis_zero(other.result),
            "Appended timer is supposed to not have frame result.");
    accum += other.accum;
    count += other.count;
  }
...
class ScopeStatTimer : public CStatTimer {
 ...
 public:
  ScopeStatTimer(CStatTimer& base, Lock& lock)
      : baseTimer(base), baseTimerLock(lock) {
    if (!g_bEnableStatGather)
      return;
    Begin();
  }
