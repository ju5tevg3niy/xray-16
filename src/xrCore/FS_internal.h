...
class CFileWriter final : public IWriter {
 ...
 public:
  CFileWriter(const char* name, bool exclusive) {
    ...
    if (exclusive) {
      const int handle = _sopen(
          conv_fn, _O_WRONLY | _O_TRUNC | _O_CREAT | _O_BINARY, SH_DENYWR);
#ifndef MASTER_GOLD
      if (handle == -1) {
        string1024 error;
...
class CTempReader final : public IReader {
 public:
  CTempReader(void* _data, size_t _size, size_t _iterpos)
...
class CPackReader final : public IReader {
 ...
 public:
  CPackReader(void* _base, void* _data, size_t _size)
      : IReader(_data, _size), base_address(_base) {}
