...
class CInifile {
 public:
  ...
  static bool isBool(pcstr str) {
    return xr_strcmp(str, "on") == 0 || xr_strcmp(str, "yes") == 0 ||
           xr_strcmp(str, "true") == 0 || xr_strcmp(str, "1") == 0;
  }
 ...
 public:
  ...
  template <typename T>
  T read_if_exists(const shared_str& section,
                   pcstr line,
  ...
  template <typename T>
  bool read_if_exists(T& outValue,
  ...
  template <typename T>
  bool try_read_if_exists(T& outValue,
                          const shared_str& section,
                          pcstr line) const {
  ...
  CLASS_ID r_clsid(const shared_str& S, pcstr L) const {
    return r_clsid(S.c_str(), L);
  }
  pcstr r_string(pcstr S, pcstr L) const;  // Left quotes in place
  pcstr r_string(const shared_str& S, pcstr L) const {
    return r_string(S.c_str(), L);
  }  // Left quotes in place
  shared_str r_string_wb(pcstr S, pcstr L) const;  // Remove quotes
  shared_str r_string_wb(const shared_str& S, pcstr L) const {
