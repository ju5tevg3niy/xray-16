...
smem_value* smem_container::dock(u32 dwCRC, u32 dwLength, void* ptr) {
  ...
  auto it =
      std::lower_bound(container.begin(), container.end(), value, smem_search);
...
void smem_container::clean() {
  ...
  for (auto& v : container)
    if (0 == v->dwReference)
      xr_free(v);
  container.erase(
      remove(container.begin(), container.end(), (smem_value*)nullptr),
      container.end());
  if (container.empty())
    container.clear();
}
...
size_t smem_container::stat_economy() const {
  ptrdiff_t counter = 0;
  {
    ...
    for (auto& v : container) {
      ...
      counter -= node_size;
      counter +=
          ptrdiff_t((ptrdiff_t(v->dwReference) - 1) * ptrdiff_t(v->dwLength));
    }
  }
