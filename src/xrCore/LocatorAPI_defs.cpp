...
// FS_File
//////////////////////////////////////////////////////////////////////
FS_File::FS_File(const std::string& nm, long sz, time_t modif, unsigned attr) {
  set(nm, sz, modif, attr);
}
FS_File::FS_File(const std::string& nm) {
  set(nm, 0, 0, 0);
}
...
// FS_Path
//////////////////////////////////////////////////////////////////////
FS_Path::FS_Path(pcstr _Root,
                 pcstr _Add,
                 pcstr _DefExt,
                 pcstr _FilterCaption,
                 u32 flags) {
  ...
  if (_Add)
    xr_strcat(temp, _Add);
  if (temp[0] && temp[strlen(temp) - 1] != _DELIMITER &&
      temp[strlen(temp) - 1] != '/')
  ...
  m_FilterCaption =
      _FilterCaption ? xr_fs_strlwr(xr_strdup(_FilterCaption)) : 0;
...
#if !defined(XR_PLATFORM_WINDOWS)  // XXX: replace with runtime check for
                                   // case-sensitivity
  string_path fullPath;
  strconcat(fullPath, m_Path, temp);
  if (FS.exist(fullPath, FSType::External) ||
      FS.exist(fullPath, FSType::Virtual)) {
